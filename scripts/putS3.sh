...
###################################################################################################################################
#
# Implements an authenticated S3 file PUT uploader
#
# - Requires the AWS CLI
# - Intended to be run from inside the production Container from a cron job to upload db backups 
# - Thanks to https://www.aws.ps/how-to-upload-file-to-s3-using-curl/
# - The key takeaway is that in the last year or so the signature required by AWS has changed and thus broke most of the other 
#   examples available on the web. 
# - While the mechanism for uploading to S3 is pretty basic the challenge is in the new signature generation that this script
#   handles. Keeping the script in it's entirety for now, mainly for the sig code. It can be refactored for our specific purposes
#   later.
#
# @usage ./putS3.sh
#
...
region=`printf $3 | awk 'BEGIN{FS="@"}{print ($2==""?"ca-central-1":$2)}'`  # region name
...
curl                            \
  ...
  -F "Content-Type=$mime"     \
  -F "file=@$srcfile"         \
  https://${bucket}.s3.ca-central-1.amazonaws.com/ | cat # pipe through cat so curl displays upload progress bar, *and* response
